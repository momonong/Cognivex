...
class AgentAConfig:
    ...
    model_path: Optional[str] = (
        "D:/hf_models/Phi-4-mini-instruct"  # Path for HuggingFace models
    )
...
class AgentA:
    ...
    def __init__(
        self, mcp_server: DiagnosticMCPServer, config: Optional[AgentAConfig] = None
    ):
        """
        Initialize Agent A
        ...
        if self.config.verbose:
            ...
            print(
                f"LLM Mode: {'Enabled' if self.config.use_llm else 'Rule-based fallback'}"
            )
            print("=" * 80)
...
Respond with JSON containing 'actions' and 'decision_rationale'."""
    ...
    def orchestrate(
        self, subject_id: str, model_name: str = "NC_MCI_AD"
    ) -> ContextObject:
        """
        ...
        if self.config.verbose:
            print(
                f"\n[AGENT A] Orchestrating analysis for {subject_id} (Model: {model_name})"
            )
        ...
        self._log_reasoning(f"Selected Model Configuration: {model_name}")

        # Choose orchestration strategy
        # [關鍵] 將 model_name 傳遞給具體的執行策略
        if self.config.use_llm:
            return self._orchestrate_with_llm(subject_id, model_name)
        else:
            return self._orchestrate_with_rules(subject_id, model_name)

    def _orchestrate_with_llm(self, subject_id: str, model_name: str) -> ContextObject:
        """
        ...
        try:
            ...
            diagnostic_report = self._read_diagnostic_report(subject_id, model_name)
            ...
            tool_results = self._execute_llm_actions(
                llm_decision["actions"], model_name
            )
            ...
            self._log_reasoning(
                f"LLM orchestration failed: {type(e).__name__}. Using rule-based fallback."
            )

            # Fallback to rule-based orchestration (Requirement 10.2)
            return self._orchestrate_with_rules(subject_id, model_name)

    def _orchestrate_with_rules(
        self, subject_id: str, model_name: str
    ) -> ContextObject:
        """
        ...
        self._log_reasoning(
            f"Evaluated signals: UQ={uq_score:.3f}, Anomaly={has_anomaly}"
        )
        ...
        if uq_score > self.config.uq_threshold:
            ...
            cf_result = self._call_counterfactual_tool(
                subject_id, top_features, model_name
            )
            tool_results["counterfactual"] = cf_result

            decision_rationale += (
                f"High uncertainty (UQ={uq_score:.3f}). Simulated counterfactual. "
            )
        ...
        if not decision_rationale:
            decision_rationale = (
                "Standard case: low uncertainty, no anomalies. Proceeding to synthesis."
            )
    ...
    def _get_llm_decision(
        self, subject_id: str, diagnostic_report: DiagnosticReport
    ) -> Dict[str, Any]:
        """
        ...
        if self.config.provider == "huggingface":
            ...
            if self.config.model not in available_models:
                if self.config.verbose:
                    ...
                    print(
                        f"[INFO] Available models: {', '.join(available_models) if available_models else 'None'}"
                    )
        ...
        user_prompt = f"""
            ...
            CRITICAL CONTEXT:
            - The underlying model is a **Binary Classifier (NC vs AD)**.
            - **MCI Detection Strategy**: Treat "Low Confidence" or "High Uncertainty" as a signal for **MCI (Out-of-Distribution)**.

            DIAGNOSTIC DATA:
            {json.dumps(diagnostic_summary, indent=2)}

            DECISION THRESHOLDS:
            - UQ Threshold: {self.config.uq_threshold} (If higher -> Trigger Counterfactual)
            - Confidence Warning: < 0.7 (If lower -> Trigger Counterfactual for OOD/MCI verification)
            - Z-Score Threshold: {self.config.z_score_threshold} (If anomaly -> Knowledge Retrieval)

            Respond with JSON containing:
            1. "actions": List of MCP actions to execute
            2. "decision_rationale": Explanation of your decisions (Mention "Potential MCI" or "OOD" if metrics are ambiguous)
        """
    ...
    def _parse_llm_response(self, response_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            parsed = parse_json_with_recovery(
                response_text, verbose=self.config.verbose
            )
            ...
            if "actions" not in parsed:
                if self.config.verbose:
                    print(
                        "[AGENT A] Response missing 'actions' field, adding empty list"
                    )
                parsed["actions"] = []
            ...
            if "decision_rationale" not in parsed:
                if self.config.verbose:
                    print(
                        "[AGENT A] Response missing 'decision_rationale' field, adding default"
                    )
    ...
    def _execute_llm_actions(
        self, actions: List[Dict], model_name: str
    ) -> Dict[str, Any]:
        """
        Execute MCP actions decided by LLM (Updated for LOOCV)

        Args:
            actions: List of actions parsed from LLM output
            model_name: The model configuration to use for any inference tools
        """
        tool_results = {}
            ...
            if action_type == "read_resource":
                ...
                if uri and uri.startswith("knowledge://"):
                    ...
                    if context:
                        ...
                        tool_results["knowledge_context"]["query_regions"].append(
                            region_name
                        )
                ...
                if tool_name == "simulate_counterfactual":
                    ...
                    # [修正] 傳遞 model_name 給 helper function
                    cf_result = self._call_counterfactual_tool(
                        subject_id, features_to_mask, model_name
                    )
                    tool_results["counterfactual"] = cf_result

        # Generate summary for knowledge context if present
        if (
            "knowledge_context" in tool_results
            and tool_results["knowledge_context"]["contexts"]
        ):
    ...
    def _read_diagnostic_report(
        self, subject_id: str, model_name: str
    ) -> DiagnosticReport:
        """
        Invoke the inference tool to generate a diagnostic report
        (Updated to support LOOCV model selection via Tool Call)

        Requirements: 3.1, 4.1
        """
        tool_name = "run_cnn_rf_inference"

        if self.config.verbose:
            print(
                f"[AGENT A] Calling tool: {tool_name} for {subject_id} (Model: {model_name})"
            )

        # [修正 1] 建立 Payload，包含 model_name
        payload = {
            "subject_id": subject_id,
            "model_name": model_name,  # <--- 關鍵參數傳遞
            "data_root": "data/MRI_processed",
        }

        # [修正 2] 建立 MCP Action (改為 call_tool)
        action = MCPAction(type="call_tool", target=tool_name, arguments=payload)

        try:
            # [修正 3] 呼叫 Tool 而不是 read_resource
            # 假設你的 mcp_server 有 call_tool 方法 (標準 MCP Client 都有)
            # 如果是本地直接呼叫，這裡會路由到我們剛剛改寫的 run_cnn_rf_inference 函數
            result = self.mcp_server.call_tool(tool_name, payload)

            if self.config.verbose:
                print(f"[DEBUG] Tool returned: {type(result)}")
            ...
            # [修正 4] 將 Tool 的 trace_log (包含 "Using LOOCV model...") 存下來
            # 這會被上層的 orchestrate 函式抓取並強制注入 Reasoning Chain
            if isinstance(result, dict) and "trace_log" in result:
                # 暫存於 ContextObject (如果這裡沒辦法直接存，上層 orchestrate 會從 mcp_actions 裡挖出來)
                pass

            self._log_reasoning(
                f"Generated diagnostic report for {subject_id} using {model_name}"
            )
            ...
            if isinstance(result, dict) and "subject_id" not in result:
                result["subject_id"] = subject_id

            # Convert to DiagnosticReport object
            # (DiagnosticReport.from_toolkit_report 應該能相容工具回傳的 dict 結構)
            diagnostic_report = DiagnosticReport.from_toolkit_report(result)

            if self.config.verbose:
                print(
                    f"[DEBUG] DiagnosticReport created: {diagnostic_report.prediction_result}"
                )
            ...
            self.mcp_actions.append(action)
            self._log_reasoning(f"Failed to generate report: {e}")
    ...
    def _read_knowledge_context(self, region_name: str) -> Optional[Dict]:
        ...
        # Create MCP action
        action = MCPAction(type="read_resource", target=uri)
    ...
    def _call_counterfactual_tool(
        ...
        model_name: str,  # [修正 1] 新增參數
    ) -> Dict:
        """
        Call counterfactual simulation tool (LOOCV-Aware)

        Requirements: 2.4, 2.5, 7.1
        """
        if self.config.verbose:
            print(f"[AGENT A] Calling tool: simulate_counterfactual")
            print(f"          Model: {model_name}")  # [Log] 記錄使用的模型
            print(f"          Masking features: {features_to_mask}")

        # [修正 2] 準備完整的參數 Payload
        payload = {
            "subject_id": subject_id,
            "features_to_mask": features_to_mask,
            "model_name": model_name,  # <--- 關鍵：指定 LOOCV 模型
            "data_root": "data/MRI_processed",
        }

        # Create MCP action
        action = MCPAction(
            type="call_tool", target="simulate_counterfactual", arguments=payload
        )

        try:
            # Call MCP server
            result = self.mcp_server.call_tool("simulate_counterfactual", payload)
            ...
            self.mcp_actions.append(action)
            self._log_reasoning(
                f"Simulated counterfactual: masked {len(features_to_mask)} features using {model_name}"
            )
            ...
            if "data" in result:
                ...
                return {
                    ...
                    if k not in ["tool", "status", "timestamp"]
                }
    ...
    def _compile_context_object(
        ...
        for action in self.mcp_actions:
            if hasattr(action, "status") and action.status == "error":
                context_object.add_error(
                    ...
                    error_message=(
                        action.error.get("message", "MCP action failed")
                        if hasattr(action, "error")
                        else "Unknown error"
                    ),
        ...
        if context_object.has_errors():
            self._log_reasoning(
                f"ContextObject contains {len(context_object.errors)} error annotation(s)"
            )
...
def demo_agent_a_rule_based():
    ...
    mcp_server = DiagnosticMCPServer(
        toolkit=toolkit, graph_rag=graph_rag, verbose=False
    )
