...
if not exist "archive\old_pipelines" mkdir archive\old_pipelines
...
REM 3.2 舊 pipeline 系統
if exist "app\core\fmri_processing\pipelines" (
    echo 移動舊 pipeline 系統...
    xcopy "app\core\fmri_processing\pipelines" "archive\old_pipelines\pipelines\" /E /I /Q >nul
    rmdir /S /Q "app\core\fmri_processing\pipelines"
    echo [完成] pipelines\ 目錄
)

REM 3.3 fmri_model_loader.py
...
echo ## 目錄結構
echo.
echo - **old_pipelines/**: 舊版 fMRI 處理 pipeline 系統
echo   - 已被 `generic_pipeline_steps.py` 取代
echo   - 保留以供參考舊實作方式
echo.
