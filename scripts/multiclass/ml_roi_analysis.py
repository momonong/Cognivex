"""
使用傳統機器學習 + ROI 特徵來分析重要腦區
這個方法更適合小樣本數據集
"""

import numpy as np
import pandas as pd
import nibabel as nib
import glob
import os
import warnings
warnings.filterwarnings('ignore')

from nilearn import datasets, image as nimg
from nilearn.maskers import NiftiLabelsMasker
from sklearn.ensemble import RandomForestClassifier
from sklearn.model_selection import StratifiedKFold, cross_val_score
from sklearn.preprocessing import StandardScaler
from sklearn.metrics import classification_report, confusion_matrix
import matplotlib.pyplot as plt
import seaborn as sns

# 配置
DATA_ROOT = "E:/fMRI/Model/sMRI_data_MultiModal_Aligned_MNI/"
OUTPUT_DIR = "output/multiclass/ml_analysis/"
os.makedirs(OUTPUT_DIR, exist_ok=True)

# AAL atlas 中與 AD 相關的重要腦區
IMPORTANT_ROIS = {
    'Hippocampus_L': 37, 'Hippocampus_R': 38,
    'ParaHippocampal_L': 39, 'ParaHippocampal_R': 40,
    'Amygdala_L': 41, 'Amygdala_R': 42,
    'Temporal_Sup_L': 79, 'Temporal_Sup_R': 80,
    'Temporal_Mid_L': 85, 'Temporal_Mid_R': 86,
    'Temporal_Inf_L': 89, 'Temporal_Inf_R': 90,
    'Parietal_Sup_L': 59, 'Parietal_Sup_R': 60,
    'Parietal_Inf_L': 61, 'Parietal_Inf_R': 62,
    'Cingulum_Ant_L': 31, 'Cingulum_Ant_R': 32,
    'Cingulum_Post_L': 35, 'Cingulum_Post_R': 36,
    'Frontal_Sup_L': 1, 'Frontal_Sup_R': 2,
    'Frontal_Mid_L': 7, 'Frontal_Mid_R': 8,
}

print("="*80)
print("機器學習 ROI 分析")
print("="*80)
print(f"使用 {len(IMPORTANT_ROIS)} 個重要 ROI")

# 1. 載入 AAL atlas
print("\n載入 AAL atlas...")
aal_atlas = datasets.fetch_atlas_aal(version='SPM12')
aal_img = nimg.load_img(aal_atlas.maps)

# 2. 建立 masker（關閉標準化，我們手動處理）
print("建立 ROI masker...")
masker = NiftiLabelsMasker(
    labels_img=aal_img,
    standardize=False,  # 關閉自動標準化
    strategy='mean'
)

# 先 fit 一次來初始化
print("初始化 masker...")
ref_t1_path = glob.glob(os.path.join(DATA_ROOT, "*", "*_T1.nii.gz"))[0]
ref_img = nimg.load_img(ref_t1_path)
masker.fit(ref_img)

# 加入 tqdm 來顯示進度
from tqdm import tqdm

# 3. 載入資料並提取特徵
print("\n載入資料並提取 ROI 特徵...")
label_map = {"NC": 0, "MCI": 1, "AD": 2}
features_list = []
labels_list = []
subject_ids = []

for label_name, label_id in label_map.items():
    class_path = os.path.join(DATA_ROOT, label_name)
    if not os.path.isdir(class_path):
        continue
    
    t1_files = glob.glob(os.path.join(class_path, "*_T1.nii.gz"))
    print(f"  {label_name}: {len(t1_files)} 個樣本")
    
    for t1_path in tqdm(t1_files, desc=f"  處理 {label_name}", leave=False):
        try:
            # 只使用 T1 (最穩定的模態)
            t1_img = nimg.load_img(t1_path)
            
            # 提取 ROI 特徵（使用 transform 而不是 fit_transform）
            roi_features = masker.transform(t1_img).flatten()
            
            # 只保留我們關心的 ROI
            important_roi_indices = [i-1 for i in IMPORTANT_ROIS.values()]  # -1 因為索引從 0 開始
            roi_features = roi_features[important_roi_indices]
            
            features_list.append(roi_features)
            labels_list.append(label_id)
            subject_ids.append(os.path.basename(t1_path).replace("_T1.nii.gz", ""))
            
        except Exception as e:
            tqdm.write(f"    錯誤：{os.path.basename(t1_path)}: {e}")

X = np.array(features_list)
y = np.array(labels_list)

print(f"\n✅ 特徵提取完成")
print(f"   特徵維度: {X.shape}")
print(f"   樣本數: NC={np.sum(y==0)}, MCI={np.sum(y==1)}, AD={np.sum(y==2)}")

# 手動標準化特徵
print("\n標準化特徵...")
scaler = StandardScaler()
X = scaler.fit_transform(X)
print(f"   特徵範圍: [{X.min():.2f}, {X.max():.2f}]")

# 4. 訓練 Random Forest
print("\n訓練 Random Forest...")
rf = RandomForestClassifier(
    n_estimators=500,
    max_depth=10,
    min_samples_split=5,
    min_samples_leaf=2,
    class_weight='balanced',
    random_state=42,
    n_jobs=-1
)

# 5-fold 交叉驗證
print("\n執行 5-fold 交叉驗證...")
kfold = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
cv_scores = cross_val_score(rf, X, y, cv=kfold, scoring='accuracy', n_jobs=-1)

print(f"\n交叉驗證結果:")
print(f"  平均準確率: {cv_scores.mean():.4f} ± {cv_scores.std():.4f}")
print(f"  各 fold: {cv_scores}")

# 6. 訓練最終模型
print("\n訓練最終模型...")
rf.fit(X, y)

# 7. 特徵重要性分析
print("\n分析特徵重要性...")
feature_importance = rf.feature_importances_
roi_names = list(IMPORTANT_ROIS.keys())

# 建立 DataFrame
importance_df = pd.DataFrame({
    'ROI': roi_names,
    'Importance': feature_importance
}).sort_values('Importance', ascending=False)

print("\n前 10 個最重要的腦區:")
print(importance_df.head(10).to_string(index=False))

# 儲存結果
importance_df.to_csv(os.path.join(OUTPUT_DIR, "roi_importance.csv"), index=False)
print(f"\n✅ 結果已儲存至: {OUTPUT_DIR}roi_importance.csv")

# 8. 視覺化
print("\n生成視覺化...")
plt.figure(figsize=(12, 8))
top_n = 15
top_rois = importance_df.head(top_n)
plt.barh(range(top_n), top_rois['Importance'].values)
plt.yticks(range(top_n), top_rois['ROI'].values)
plt.xlabel('Feature Importance')
plt.title(f'Top {top_n} Most Important Brain Regions')
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, "roi_importance.png"), dpi=300, bbox_inches='tight')
print(f"✅ 圖表已儲存至: {OUTPUT_DIR}roi_importance.png")

print("\n" + "="*80)
print("分析完成！")
print("="*80)
print(f"\n結果摘要:")
print(f"  - 模型準確率: {cv_scores.mean():.2%}")
print(f"  - 最重要的腦區: {importance_df.iloc[0]['ROI']}")
print(f"  - 詳細結果: {OUTPUT_DIR}")
