...
from agents.sub_agents.act_to_brain.tools.pipeline_tools import (
    ...
    LABEL_PATH,
    NORM_TYPE,
    ACT_THRESHOLD_PERCENTILE,
    VIS_THRESHOLD_PERCENTILE,
)
        ...
        for i, layer_name in enumerate(final_layer_names):
            ...
            try:
                ...
                resampled_path = resample_to_atlas(
                    act_path=nii_output, atlas_path=ATLAS_PATH, output_dir=vis_dir
                )
                ...
                vis_output_path = visualize_activation_map_data(
                    ...
                    output_path=os.path.join(
                        vis_dir, f"activation_map_{layer_name.replace('.', '_')}.png"
                    ),
...
def pipeline(subject_id: str, model_path: str, nii_path: str) -> str:
    """
    ...
    return complete_nii_inference_pipeline(
        subject_id=subject_id, model_path=model_path, nii_path=nii_path
    )
